...
const BrandsManagementPage = () => {
  const { brands, addBrand, updateBrand, deleteBrand } = useDataManagement();
  const { user } = useAuth();
  ...
  const handleSaveEdit = async () => {
    ...
    const oldName = brands[editingIndex!];

    if (trimmedName !== oldName && brands.includes(trimmedName)) {
      toast({
        title: "Erro",
        description: "Esta marca já existe",
        variant: "destructive",
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Marcas Cadastradas ({filteredBrands.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Tem certeza que deseja excluir a marca "{brand}"? Esta ação não pode ser desfeita.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
